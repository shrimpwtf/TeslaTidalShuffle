...
func (s *Service) DeletePlaylist(pid string) error {
	log.Debugf("Deleting playlist %s", pid)

	// HTTP
	client := &http.Client{}

	req, err := http.NewRequest("PUT", fmt.Sprintf("%s/my-collection/playlists/folders/create-remove", apiURL2), nil)
	if err != nil {
		return err
	}
